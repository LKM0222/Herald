 ...
 *   점수로 바꾸면 개수는 결과가 된다. 문턱을 넘는 게 없는 날은 1층이 빈다. 그게 맞다.
 *
 * ⚠ 그래도 상한을 남겨 둔 이유: LLM 점수는 **교정되지 않는다.** 같은 기준을
 *   줘도 그날 목록 구성에 따라 전체가 위아래로 밀린다. 시끄러운 날 1층이
 ...
 */
const TIERS = [
  { priority: 1 as const, min: 85 },
  { priority: 2 as const, min: 70 },
  { priority: 3 as const, min: 55 },
];
 ...
 * 영역별 화면 정원과 원문 읽기 건수.
 *
 * ⚠ 경계(85 / 70 / 55)는 네 영역이 함께 쓰지만 **정원은 영역마다 다르다.**
 *   개발이 주력이라 넓고 나머지는 좁다. 이건 "얼마나 중요한가" 가 아니라
 *   **화면을 얼마나 길게 쓸까** 의 값이다.
 *
 * ⚠ **최소치가 아니라 상한이다.** 그날 게임에 문턱을 넘는 게 셋뿐이면 셋만 뜬다.
...
export async function summarize(items: NewsItem[]): Promise<SummarizeResult> {
  ...
  /* 전체 목록은 **영역 순서대로 이어 붙인다.** 점수로 다시 세우지 않는다 —
     영역마다 다른 기준으로 매긴 값이라 개발 82 와 경제 82 는 견줄 수 없다.
...
⚠ 점수를 후하게 주지 마라. 85점 이상은 그날 **가장 위**에 놓일 것이다.
  흥미로운 것과 그만한 무게가 있는 것은 다르다. 하루에 85점 이상이 하나도 없는 날이
  이상한 날이 아니다. 그런 날이 대부분이다.

⚠ 개수를 맞추려 하지 마라. 85점 이상이 하나뿐이면 하나만 적는다.
