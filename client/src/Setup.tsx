    ...
    setError(
      result.kind === "wrong"
        ...
        : result.kind === "throttled"
          ? `시도가 너무 많아요. ${result.retryAfter}초 뒤에 다시 해주세요.`
          : `서버에 닿지 않아요 (${result.message})`,
    );
  }
  ...
  return (
    <main className="flex flex-1 items-center justify-center px-6 py-10">
      ...
      >
        ...
        <Field label={mode === "password" ? "비밀번호" : "API 토큰"}>
          <input
            value={secret}
        ...
        {error ? (
          <p className="rounded-xl border border-accent px-3 py-2 text-xs leading-relaxed text-accent">
            {error}
          </p>
        ) : null}
        ...
        <div className="flex gap-2">
          ...
          <Button type="submit" variant="primary" disabled={!ready} className="flex-1">
            {busy ? "확인 중…" : "연결"}
          </Button>
        </div>
        ...
        <button
          ...
          className="text-xs text-dim underline underline-offset-2 hover:text-fg"
        >
          {mode === "password"
            ? "비밀번호 대신 API 토큰으로 연결"
            : "비밀번호로 연결"}
        ...
        <p className="text-xs leading-relaxed text-dim">
          이 브라우저에만 저장돼요. 비밀번호는 연결할 때 한 번만 쓰이고,
          이후에는 서버가 발급한 토큰으로 통신합니다.
        </p>

        {/* 아직 연결 전이라도 밝기는 고를 수 있어야 한다 */}
